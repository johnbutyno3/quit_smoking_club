...
class _SudokuPageState extends State<SudokuPage> {
  ...
  void _enterNumber(int value) {
    ...
    if (value == _solution[row][col]) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Wrong number'), duration: Duration(milliseconds: 700),
      ));
    }
  }
  ...
  void _useHint() {
    ...
    final row = _selectedRow;
    final col = _selectedCol;
    setState(() { _board[row][col] = _solution[row][col]; _hints--; });
    _checkComplete();
  }
  ...
  void _checkComplete() {
    ...
    showDialog<void>(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          TextButton(onPressed: () { Navigator.pop(context); _newGame(); }, child: const Text('New Game')),
        ],
      ),
    );
  }
  ...
  Widget _cell(int row, int col) {
    ...
    final sameValue = _selectedRow >= 0 && _selectedCol >= 0 &&
        _board[row][col] != 0 && _board[row][col] == _board[_selectedRow][_selectedCol];
      ...
      Row(children: List.generate(9, (index) => Expanded(child: Padding(
        ...
        child: ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(
            elevation: 0, backgroundColor: Colors.white, foregroundColor: const Color(0xFF1B5E20),
